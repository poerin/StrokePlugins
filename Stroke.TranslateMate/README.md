...
A clipboard-driven translation plugin for [Stroke](https://github.com/poerin/Stroke). It automatically detects Chinese or English text, queries the Baidu Translation API, and displays the result in a draggable overlay. The plugin also provides pronunciation via Youdao Dictionary and maintains a vocabulary notebook.
...
- A valid Baidu Translation API account.

## API Credentials

### Baidu Translation API

This plugin uses the **Baidu Universal Translation API**.
...
A typical translation gesture script is shown below. **Before executing the gesture, first select the text you want to translate.** The script will simulate `Ctrl+C` to copy the selection to the clipboard; the plugin detects the new clipboard content and performs the translation automatically.

```csharp
TranslateMate.AppId = "your_app_id";
